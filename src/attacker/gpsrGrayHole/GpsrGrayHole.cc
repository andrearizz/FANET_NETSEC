...
void GpsrGrayHole::initialize(int stage)
{
...
void GpsrGrayHole::processSelfMessage(cMessage *message)
{
...
void GpsrGrayHole::processMessage(cMessage *message)
{
...
void GpsrGrayHole::schedulePurgeNeighborsTimer()
{
...
void GpsrGrayHole::processPurgeNeighborsTimer()
{
...
void GpsrGrayHole::sendUdpPacket(Packet *packet)
{
    send(packet, "ipOut");
}

void GpsrGrayHole::processUdpPacket(Packet *packet)
{
...
const Ptr<GpsrBeacon> GpsrGrayHole::createBeacon()
{
...
GpsrOption* GpsrGrayHole::createGpsrOption(L3Address destination)
{
...
int GpsrGrayHole::computeOptionLength(GpsrOption *option)
{
    // routingMode
...
void GpsrGrayHole::configureInterfaces()
{
...
// KLUDGE: implement position registry protocol
PositionTable GpsrGrayHole::globalPositionTable;

Coord GpsrGrayHole::lookupPositionInGlobalRegistry(const L3Address& address) const
{
    // KLUDGE: implement position registry protocol
...
void GpsrGrayHole::storePositionInGlobalRegistry(const L3Address& address, const Coord& position) const
{
    // KLUDGE: implement position registry protocol
    globalPositionTable.setPosition(address, position);
}

void GpsrGrayHole::storeSelfPositionInGlobalRegistry() const
{
...
Coord GpsrGrayHole::computeIntersectionInsideLineSegments(Coord& begin1, Coord& end1, Coord& begin2, Coord& end2) const
{
...
Coord GpsrGrayHole::getNeighborPosition(const L3Address& address) const
{
...
double GpsrGrayHole::getVectorAngle(Coord vector) const
{
...
double GpsrGrayHole::getNeighborAngle(const L3Address& address) const
{
...
std::string GpsrGrayHole::getHostName() const
{
    return host->getFullName();
}

L3Address GpsrGrayHole::getSelfAddress() const
{
...
L3Address GpsrGrayHole::getSenderNeighborAddress(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
...
simtime_t GpsrGrayHole::getNextNeighborExpiration()
{
...
void GpsrGrayHole::purgeNeighbors()
{
    neighborPositionTable.removeOldPositions(simTime() - neighborValidityInterval);
}

std::vector<L3Address> GpsrGrayHole::getPlanarNeighbors() const
{
...
std::vector<L3Address> GpsrGrayHole::getPlanarNeighborsCounterClockwise(double startAngle) const
{
...
L3Address GpsrGrayHole::findGreedyRoutingNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
L3Address GpsrGrayHole::findPerimeterRoutingNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
INetfilter::IHook::Result GpsrGrayHole::routeDatagram(Packet *datagram, GpsrOption *gpsrOption)
{
...
void GpsrGrayHole::setGpsrOptionOnNetworkDatagram(Packet *packet, const Ptr<const NetworkHeaderBase>& networkHeader, GpsrOption *gpsrOption)
{
    packet->trimFront();
#ifdef WITH_IPv4
...
GpsrOption *GpsrGrayHole::findGpsrOptionInNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
...
const GpsrOption *GpsrGrayHole::getGpsrOptionFromNetworkDatagram(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
    const GpsrOption *gpsrOption = findGpsrOptionInNetworkDatagram(networkHeader);
    if (gpsrOption == nullptr)
        throw cRuntimeError("GpsrGrayHole option not found in datagram!");
    return gpsrOption;
}

GpsrOption *GpsrGrayHole::getGpsrOptionFromNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
...
INetfilter::IHook::Result GpsrGrayHole::datagramPreRoutingHook(Packet *datagram)
{
...
INetfilter::IHook::Result GpsrGrayHole::datagramLocalOutHook(Packet *packet)
{
...
void GpsrGrayHole::handleStartOperation(LifecycleOperation *operation)
{
    configureInterfaces();
    storeSelfPositionInGlobalRegistry();
    scheduleBeaconTimer();
...
void GpsrGrayHole::handleStopOperation(LifecycleOperation *operation)
{
...
void GpsrGrayHole::handleCrashOperation(LifecycleOperation *operation)
{
    neighborPositionTable.clear();
    cancelEvent(beaconTimer);
    cancelEvent(purgeNeighborsTimer);
...
void GpsrGrayHole::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
