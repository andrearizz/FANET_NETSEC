...
#include "GpsrCoordinateNormale.h"
...
Define_Module(GpsrCoordinateNormale);
...
GpsrCoordinateNormale::GpsrCoordinateNormale()
{
}
...
void GpsrCoordinateNormale::initialize(int stage)
{
    srand(420);
    ...
    if (stage == INITSTAGE_LOCAL) {
        // GpsrCoordinateNormale parameters
...
void GpsrCoordinateNormale::handleMessageWhenUp(cMessage *message)
{
...
void GpsrCoordinateNormale::processSelfMessage(cMessage *message)
{
...
void GpsrCoordinateNormale::processMessage(cMessage *message)
{
...
void GpsrCoordinateNormale::processBeaconTimer()
{
    EV_DEBUG << "Processing beacon timer" << endl;
        ...
        if (!selfAddress.isUnspecified()) {
            sendBeacon(createBeacon(("host["+std::to_string(rand() % 25) + "]").c_str()));
            //sendBeacon(createBeacon(("host["+std::to_string(2) + "]").c_str()));
...
void GpsrCoordinateNormale::schedulePurgeNeighborsTimer()
{
...
void GpsrCoordinateNormale::processPurgeNeighborsTimer()
{
...
void GpsrCoordinateNormale::sendUdpPacket(Packet *packet)
{
    // std::cout << "------------Attaccante--------------" << endl;
    send(packet, "ipOut");
}

void GpsrCoordinateNormale::processUdpPacket(Packet *packet)
{
...
const Ptr<GpsrBeacon> GpsrCoordinateNormale::createBeacon(const char* addressStr)
{
    ...
    double y = f1 * 2001;
    double z = 40 + f2 * 161;
    const Coord c = mobility -> getCurrentPosition();
    /*
    std::cout << "X = " << x << " Y = " << y << " Z = " << z << endl;
    std::cout << "COORDINATE REALI" << endl;
    std::cout << "X = " << c.getX() << " Y = " << c.getY() << " Z = " << c.getZ() << endl;
    */
...
void GpsrCoordinateNormale::sendBeacon(const Ptr<GpsrBeacon>& beacon)
{
    EV_INFO << "Sending sybil beacon: address = " << beacon->getAddress() << ", position = " << beacon->getPosition() << endl;
    Packet *udpPacket = new Packet("GpsrCoordinateNormale");
...
void GpsrCoordinateNormale::processBeacon(Packet *packet)
{
...
GpsrOption* GpsrCoordinateNormale::createGpsrOption(L3Address destination)
{
...
int GpsrCoordinateNormale::computeOptionLength(GpsrOption *option)
{
    // routingMode
...
void GpsrCoordinateNormale::configureInterfaces()
{
...
// KLUDGE: implement position registry protocol
PositionTable GpsrCoordinateNormale::globalPositionTable;

Coord GpsrCoordinateNormale::lookupPositionInGlobalRegistry(const L3Address& address) const
{
    // KLUDGE: implement position registry protocol
    return globalPositionTable.getPosition(address);
}

void GpsrCoordinateNormale::storePositionInGlobalRegistry(const L3Address& address, const Coord& position) const
{
    // KLUDGE: implement position registry protocol
    globalPositionTable.setPosition(address, position);
}

void GpsrCoordinateNormale::storeSelfPositionInGlobalRegistry() const
{
...
Coord GpsrCoordinateNormale::computeIntersectionInsideLineSegments(Coord& begin1, Coord& end1, Coord& begin2, Coord& end2) const
{
...
Coord GpsrCoordinateNormale::getNeighborPosition(const L3Address& address) const
{
...
double GpsrCoordinateNormale::getVectorAngle(Coord vector) const
{
...
double GpsrCoordinateNormale::getNeighborAngle(const L3Address& address) const
{
...
std::string GpsrCoordinateNormale::getHostName() const
{
    return host->getFullName();
}

L3Address GpsrCoordinateNormale::getSelfAddress() const
{
...
L3Address GpsrCoordinateNormale::getSenderNeighborAddress(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
...
simtime_t GpsrCoordinateNormale::getNextNeighborExpiration()
{
...
void GpsrCoordinateNormale::purgeNeighbors()
{
    neighborPositionTable.removeOldPositions(simTime() - neighborValidityInterval);
}

std::vector<L3Address> GpsrCoordinateNormale::getPlanarNeighbors() const
{
...
std::vector<L3Address> GpsrCoordinateNormale::getPlanarNeighborsCounterClockwise(double startAngle) const
{
...
L3Address GpsrCoordinateNormale::findNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
L3Address GpsrCoordinateNormale::findGreedyRoutingNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
L3Address GpsrCoordinateNormale::findPerimeterRoutingNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
INetfilter::IHook::Result GpsrCoordinateNormale::routeDatagram(Packet *datagram, GpsrOption *gpsrOption)
{
...
void GpsrCoordinateNormale::setGpsrOptionOnNetworkDatagram(Packet *packet, const Ptr<const NetworkHeaderBase>& networkHeader, GpsrOption *gpsrOption)
{
    packet->trimFront();
#ifdef WITH_IPv4
...
const GpsrOption *GpsrCoordinateNormale::findGpsrOptionInNetworkDatagram(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
...
GpsrOption *GpsrCoordinateNormale::findGpsrOptionInNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
...
const GpsrOption *GpsrCoordinateNormale::getGpsrOptionFromNetworkDatagram(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
    const GpsrOption *gpsrOption = findGpsrOptionInNetworkDatagram(networkHeader);
    if (gpsrOption == nullptr)
        throw cRuntimeError("GpsrCoordinateNormale option not found in datagram!");
    return gpsrOption;
}

GpsrOption *GpsrCoordinateNormale::getGpsrOptionFromNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
...
INetfilter::IHook::Result GpsrCoordinateNormale::datagramPreRoutingHook(Packet *datagram)
{
...
INetfilter::IHook::Result GpsrCoordinateNormale::datagramLocalOutHook(Packet *packet)
{
...
void GpsrCoordinateNormale::handleStartOperation(LifecycleOperation *operation)
{
    configureInterfaces();
    storeSelfPositionInGlobalRegistry();
    scheduleBeaconTimer();
...
void GpsrCoordinateNormale::handleStopOperation(LifecycleOperation *operation)
{
...
void GpsrCoordinateNormale::handleCrashOperation(LifecycleOperation *operation)
{
    neighborPositionTable.clear();
    cancelEvent(beaconTimer);
    cancelEvent(purgeNeighborsTimer);
...
void GpsrCoordinateNormale::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
