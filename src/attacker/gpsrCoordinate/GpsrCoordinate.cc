...
void GpsrCoordinate::initialize(int stage)
{
    srand(420);
...
void GpsrCoordinate::handleMessageWhenUp(cMessage *message)
{
...
void GpsrCoordinate::processSelfMessage(cMessage *message)
{
...
void GpsrCoordinate::processMessage(cMessage *message)
{
...
void GpsrCoordinate::schedulePurgeNeighborsTimer()
{
...
void GpsrCoordinate::processPurgeNeighborsTimer()
{
...
void GpsrCoordinate::sendUdpPacket(Packet *packet)
{
    // std::cout << "------------Attaccante--------------" << endl;
    send(packet, "ipOut");
}

void GpsrCoordinate::processUdpPacket(Packet *packet)
{
...
const Ptr<GpsrBeaconSecure> GpsrCoordinate::createBeacon(const char* addressStr)
{
...
void GpsrCoordinate::sendBeacon(const Ptr<GpsrBeaconSecure>& beacon)
{
    EV_INFO << "Sending sybil beacon: address = " << beacon->getAddress() << ", position = " << beacon->getPosition() << endl;
    Packet *udpPacket = new Packet("GpsrCoordinateSecure");
...
void GpsrCoordinate::processBeacon(Packet *packet)
{
...
GpsrOption* GpsrCoordinate::createGpsrOption(L3Address destination)
{
...
int GpsrCoordinate::computeOptionLength(GpsrOption *option)
{
    // routingMode
...
void GpsrCoordinate::configureInterfaces()
{
...
// KLUDGE: implement position registry protocol
PositionTable GpsrCoordinate::globalPositionTable;

Coord GpsrCoordinate::lookupPositionInGlobalRegistry(const L3Address& address) const
{
    // KLUDGE: implement position registry protocol
    return globalPositionTable.getPosition(address);
}

void GpsrCoordinate::storePositionInGlobalRegistry(const L3Address& address, const Coord& position) const
{
    // KLUDGE: implement position registry protocol
    globalPositionTable.setPosition(address, position);
}

void GpsrCoordinate::storeSelfPositionInGlobalRegistry() const
{
...
Coord GpsrCoordinate::computeIntersectionInsideLineSegments(Coord& begin1, Coord& end1, Coord& begin2, Coord& end2) const
{
...
Coord GpsrCoordinate::getNeighborPosition(const L3Address& address) const
{
...
double GpsrCoordinate::getVectorAngle(Coord vector) const
{
...
double GpsrCoordinate::getNeighborAngle(const L3Address& address) const
{
...
std::string GpsrCoordinate::getHostName() const
{
    return host->getFullName();
}

L3Address GpsrCoordinate::getSelfAddress() const
{
...
L3Address GpsrCoordinate::getSenderNeighborAddress(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
...
simtime_t GpsrCoordinate::getNextNeighborExpiration()
{
...
void GpsrCoordinate::purgeNeighbors()
{
    neighborPositionTable.removeOldPositions(simTime() - neighborValidityInterval);
}

std::vector<L3Address> GpsrCoordinate::getPlanarNeighbors() const
{
...
std::vector<L3Address> GpsrCoordinate::getPlanarNeighborsCounterClockwise(double startAngle) const
{
...
L3Address GpsrCoordinate::findNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
L3Address GpsrCoordinate::findGreedyRoutingNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
L3Address GpsrCoordinate::findPerimeterRoutingNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
...
INetfilter::IHook::Result GpsrCoordinate::routeDatagram(Packet *datagram, GpsrOption *gpsrOption)
{
...
void GpsrCoordinate::setGpsrOptionOnNetworkDatagram(Packet *packet, const Ptr<const NetworkHeaderBase>& networkHeader, GpsrOption *gpsrOption)
{
    packet->trimFront();
#ifdef WITH_IPv4
...
const GpsrOption *GpsrCoordinate::findGpsrOptionInNetworkDatagram(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
...
GpsrOption *GpsrCoordinate::findGpsrOptionInNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
...
const GpsrOption *GpsrCoordinate::getGpsrOptionFromNetworkDatagram(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
    const GpsrOption *gpsrOption = findGpsrOptionInNetworkDatagram(networkHeader);
    if (gpsrOption == nullptr)
        throw cRuntimeError("GpsrCoordinate option not found in datagram!");
    return gpsrOption;
}

GpsrOption *GpsrCoordinate::getGpsrOptionFromNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
...
INetfilter::IHook::Result GpsrCoordinate::datagramPreRoutingHook(Packet *datagram)
{
...
INetfilter::IHook::Result GpsrCoordinate::datagramLocalOutHook(Packet *packet)
{
...
void GpsrCoordinate::handleStartOperation(LifecycleOperation *operation)
{
    configureInterfaces();
    storeSelfPositionInGlobalRegistry();
    scheduleBeaconTimer();
...
void GpsrCoordinate::handleStopOperation(LifecycleOperation *operation)
{
...
void GpsrCoordinate::handleCrashOperation(LifecycleOperation *operation)
{
    neighborPositionTable.clear();
    cancelEvent(beaconTimer);
    cancelEvent(purgeNeighborsTimer);
...
void GpsrCoordinate::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
