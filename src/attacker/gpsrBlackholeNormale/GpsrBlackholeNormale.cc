...
#include "GpsrBlackholeNormaleBlackholeNormale.h"
...
Define_Module(GpsrBlackholeNormale);
...
GpsrBlackholeNormale::GpsrBlackholeNormale()
{
}
...
void GpsrBlackholeNormale::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (!strcmp(planarizationModeString, ""))
            planarizationMode = GpsrBlackholeNormale_NO_PLANARIZATION;
        else if (!strcmp(planarizationModeString, "GG"))
            planarizationMode = GpsrBlackholeNormale_GG_PLANARIZATION;
        else if (!strcmp(planarizationModeString, "RNG"))
            planarizationMode = GpsrBlackholeNormale_RNG_PLANARIZATION;
        else
...
void GpsrBlackholeNormale::handleMessageWhenUp(cMessage *message)
{
...
void GpsrBlackholeNormale::processSelfMessage(cMessage *message)
{
...
void GpsrBlackholeNormale::processMessage(cMessage *message)
{
...
void GpsrBlackholeNormale::schedulePurgeNeighborsTimer()
{
...
void GpsrBlackholeNormale::processPurgeNeighborsTimer()
{
...
void GpsrBlackholeNormale::sendUdpPacket(Packet *packet)
{
    send(packet, "ipOut");
}

void GpsrBlackholeNormale::processUdpPacket(Packet *packet)
{
...
const Ptr<GpsrBlackholeNormaleBeacon> GpsrBlackholeNormale::createBeacon()
{
    const auto& beacon = makeShared<GpsrBlackholeNormaleBeacon>();
...
void GpsrBlackholeNormale::sendBeacon(const Ptr<GpsrBlackholeNormaleBeacon>& beacon)
{
    EV_INFO << "Sending beacon: address = " << beacon->getAddress() << ", position = " << beacon->getPosition() << endl;
    Packet *udpPacket = new Packet("GpsrBlackholeNormaleBeacon");
    udpPacket->insertAtBack(beacon);
    auto udpHeader = makeShared<UdpHeader>();
    udpHeader->setSourcePort(GpsrBlackholeNormale_UDP_PORT);
    udpHeader->setDestinationPort(GpsrBlackholeNormale_UDP_PORT);
...
void GpsrBlackholeNormale::processBeacon(Packet *packet)
{
    const auto& beacon = packet->peekAtFront<GpsrBlackholeNormaleBeacon>();
...
GpsrBlackholeNormaleOption* GpsrBlackholeNormale::createGpsrBlackholeNormaleOption(L3Address destination)
{
    GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption = new GpsrBlackholeNormaleOption();
    GpsrBlackholeNormaleOption->setRoutingMode(GpsrBlackholeNormale_GREEDY_ROUTING);
    GpsrBlackholeNormaleOption->setDestinationPosition(lookupPositionInGlobalRegistry(destination));
    GpsrBlackholeNormaleOption->setLength(computeOptionLength(GpsrBlackholeNormaleOption));
    return GpsrBlackholeNormaleOption;
}

int GpsrBlackholeNormale::computeOptionLength(GpsrBlackholeNormaleOption *option)
{
    // routingMode
...
void GpsrBlackholeNormale::configureInterfaces()
{
...
// KLUDGE: implement position registry protocol
PositionTable GpsrBlackholeNormale::globalPositionTable;

Coord GpsrBlackholeNormale::lookupPositionInGlobalRegistry(const L3Address& address) const
{
    // KLUDGE: implement position registry protocol
...
void GpsrBlackholeNormale::storePositionInGlobalRegistry(const L3Address& address, const Coord& position) const
{
    // KLUDGE: implement position registry protocol
    globalPositionTable.setPosition(address, position);
}

void GpsrBlackholeNormale::storeSelfPositionInGlobalRegistry() const
{
...
Coord GpsrBlackholeNormale::computeIntersectionInsideLineSegments(Coord& begin1, Coord& end1, Coord& begin2, Coord& end2) const
{
...
Coord GpsrBlackholeNormale::getNeighborPosition(const L3Address& address) const
{
...
double GpsrBlackholeNormale::getVectorAngle(Coord vector) const
{
...
double GpsrBlackholeNormale::getNeighborAngle(const L3Address& address) const
{
...
std::string GpsrBlackholeNormale::getHostName() const
{
    return host->getFullName();
}

L3Address GpsrBlackholeNormale::getSelfAddress() const
{
...
L3Address GpsrBlackholeNormale::getSenderNeighborAddress(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
    const GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption = getGpsrBlackholeNormaleOptionFromNetworkDatagram(networkHeader);
    return GpsrBlackholeNormaleOption->getSenderAddress();
}
...
simtime_t GpsrBlackholeNormale::getNextNeighborExpiration()
{
...
void GpsrBlackholeNormale::purgeNeighbors()
{
    neighborPositionTable.removeOldPositions(simTime() - neighborValidityInterval);
}

std::vector<L3Address> GpsrBlackholeNormale::getPlanarNeighbors() const
{
    ...
    for (auto it = neighborAddresses.begin(); it != neighborAddresses.end(); it++) {
        ...
        if (planarizationMode == GpsrBlackholeNormale_NO_PLANARIZATION)
            return neighborAddresses;
        else if (planarizationMode == GpsrBlackholeNormale_RNG_PLANARIZATION) {
        ...
        else if (planarizationMode == GpsrBlackholeNormale_GG_PLANARIZATION) {
...
std::vector<L3Address> GpsrBlackholeNormale::getPlanarNeighborsCounterClockwise(double startAngle) const
{
...
L3Address GpsrBlackholeNormale::findNextHop(const L3Address& destination, GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption)
{
    switch (GpsrBlackholeNormaleOption->getRoutingMode()) {
        case GpsrBlackholeNormale_GREEDY_ROUTING: return findGreedyRoutingNextHop(destination, GpsrBlackholeNormaleOption);
        case GpsrBlackholeNormale_PERIMETER_ROUTING: return findPerimeterRoutingNextHop(destination, GpsrBlackholeNormaleOption);
        default: throw cRuntimeError("Unknown routing mode");
    }
}

L3Address GpsrBlackholeNormale::findGreedyRoutingNextHop(const L3Address& destination, GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption)
{
    EV_DEBUG << "Finding next hop using greedy routing: destination = " << destination << endl;
    L3Address selfAddress = getSelfAddress();
    Coord selfPosition = mobility->getCurrentPosition();
    Coord destinationPosition = GpsrBlackholeNormaleOption->getDestinationPosition();
    ...
    if (bestNeighbor.isUnspecified()) {
        ...
        GpsrBlackholeNormaleOption->setRoutingMode(GpsrBlackholeNormale_PERIMETER_ROUTING);
        GpsrBlackholeNormaleOption->setPerimeterRoutingStartPosition(selfPosition);
        GpsrBlackholeNormaleOption->setPerimeterRoutingForwardPosition(selfPosition);
        GpsrBlackholeNormaleOption->setCurrentFaceFirstSenderAddress(selfAddress);
        GpsrBlackholeNormaleOption->setCurrentFaceFirstReceiverAddress(L3Address());
        return findPerimeterRoutingNextHop(destination, GpsrBlackholeNormaleOption);
    }
    else
        return bestNeighbor;
}

L3Address GpsrBlackholeNormale::findPerimeterRoutingNextHop(const L3Address& destination, GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption)
{
    EV_DEBUG << "Finding next hop using perimeter routing: destination = " << destination << endl;
    L3Address selfAddress = getSelfAddress();
    Coord selfPosition = mobility->getCurrentPosition();
    Coord perimeterRoutingStartPosition = GpsrBlackholeNormaleOption->getPerimeterRoutingStartPosition();
    Coord destinationPosition = GpsrBlackholeNormaleOption->getDestinationPosition();
    ...
    if (selfDistance < perimeterRoutingStartDistance) {
        ...
        GpsrBlackholeNormaleOption->setRoutingMode(GpsrBlackholeNormale_GREEDY_ROUTING);
        GpsrBlackholeNormaleOption->setPerimeterRoutingStartPosition(Coord());
        GpsrBlackholeNormaleOption->setPerimeterRoutingForwardPosition(Coord());
        GpsrBlackholeNormaleOption->setCurrentFaceFirstSenderAddress(L3Address());
        GpsrBlackholeNormaleOption->setCurrentFaceFirstReceiverAddress(L3Address());
        return findGreedyRoutingNextHop(destination, GpsrBlackholeNormaleOption);
    }
    else {
        const L3Address& firstSenderAddress = GpsrBlackholeNormaleOption->getCurrentFaceFirstSenderAddress();
        const L3Address& firstReceiverAddress = GpsrBlackholeNormaleOption->getCurrentFaceFirstReceiverAddress();
        auto senderNeighborAddress = GpsrBlackholeNormaleOption->getSenderAddress();
        ...
        for (auto& neighborAddress : neighborAddresses) {
            ...
            if (std::isnan(intersection.x)) {
                ...
                GpsrBlackholeNormaleOption->setCurrentFaceFirstSenderAddress(selfAddress);
                GpsrBlackholeNormaleOption->setCurrentFaceFirstReceiverAddress(L3Address());
                GpsrBlackholeNormaleOption->setPerimeterRoutingForwardPosition(intersection);
            }
        }
        if (selectedNeighborAddress.isUnspecified()) {
            ...
            if (GpsrBlackholeNormaleOption->getCurrentFaceFirstReceiverAddress().isUnspecified())
                GpsrBlackholeNormaleOption->setCurrentFaceFirstReceiverAddress(selectedNeighborAddress);
...
INetfilter::IHook::Result GpsrBlackholeNormale::routeDatagram(Packet *datagram, GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption)
{
    ...
    auto nextHop = findNextHop(destination, GpsrBlackholeNormaleOption);
    ...
    if (nextHop.isUnspecified()) {
        ...
        GpsrBlackholeNormaleOption->setSenderAddress(getSelfAddress());
...
void GpsrBlackholeNormale::setGpsrBlackholeNormaleOptionOnNetworkDatagram(Packet *packet, const Ptr<const NetworkHeaderBase>& networkHeader, GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption)
{
    packet->trimFront();
#ifdef WITH_IPv4
    if (dynamicPtrCast<const Ipv4Header>(networkHeader)) {
        auto ipv4Header = removeNetworkProtocolHeader<Ipv4Header>(packet);
        GpsrBlackholeNormaleOption->setType(IPOPTION_TLV_GpsrBlackholeNormale);
        B oldHlen = ipv4Header->calculateHeaderByteLength();
        ASSERT(ipv4Header->getHeaderLength() == oldHlen);
        ipv4Header->addOption(GpsrBlackholeNormaleOption);
    ...
    if (dynamicPtrCast<const Ipv6Header>(networkHeader)) {
        ...
        GpsrBlackholeNormaleOption->setType(IPv6TLVOPTION_TLV_GpsrBlackholeNormale);
        ...
        hdr->getTlvOptionsForUpdate().appendTlvOption(GpsrBlackholeNormaleOption);
    ...
    if (dynamicPtrCast<const NextHopForwardingHeader>(networkHeader)) {
        ...
        GpsrBlackholeNormaleOption->setType(NEXTHOP_TLVOPTION_TLV_GpsrBlackholeNormale);
        int oldHlen = nextHopHeader->getTlvOptions().getLength();
        nextHopHeader->getTlvOptionsForUpdate().appendTlvOption(GpsrBlackholeNormaleOption);
...
const GpsrBlackholeNormaleOption *GpsrBlackholeNormale::findGpsrBlackholeNormaleOptionInNetworkDatagram(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
    const GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption = nullptr;

#ifdef WITH_IPv4
    if (auto ipv4Header = dynamicPtrCast<const Ipv4Header>(networkHeader)) {
        GpsrBlackholeNormaleOption = check_and_cast_nullable<const GpsrBlackholeNormaleOption *>(ipv4Header->findOptionByType(IPOPTION_TLV_GpsrBlackholeNormale));
    }
    else
#endif
#ifdef WITH_IPv6
    if (auto ipv6Header = dynamicPtrCast<const Ipv6Header>(networkHeader)) {
        ...
        if (hdr != nullptr) {
            int i = (hdr->getTlvOptions().findByType(IPv6TLVOPTION_TLV_GpsrBlackholeNormale));
            if (i >= 0)
                GpsrBlackholeNormaleOption = check_and_cast<const GpsrBlackholeNormaleOption *>(hdr->getTlvOptions().getTlvOption(i));
        }
    }
    else
#endif
    ...
    if (auto nextHopHeader = dynamicPtrCast<const NextHopForwardingHeader>(networkHeader)) {
        int i = (nextHopHeader->getTlvOptions().findByType(NEXTHOP_TLVOPTION_TLV_GpsrBlackholeNormale));
        if (i >= 0)
            GpsrBlackholeNormaleOption = check_and_cast<const GpsrBlackholeNormaleOption *>(nextHopHeader->getTlvOptions().getTlvOption(i));
    }
    else
#endif
    {
...
GpsrBlackholeNormaleOption *GpsrBlackholeNormale::findGpsrBlackholeNormaleOptionInNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
    GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption = nullptr;

#ifdef WITH_IPv4
    if (auto ipv4Header = dynamicPtrCast<Ipv4Header>(networkHeader)) {
        GpsrBlackholeNormaleOption = check_and_cast_nullable<GpsrBlackholeNormaleOption *>(ipv4Header->findMutableOptionByType(IPOPTION_TLV_GpsrBlackholeNormale));
    }
    else
#endif
#ifdef WITH_IPv6
    if (auto ipv6Header = dynamicPtrCast<Ipv6Header>(networkHeader)) {
        ...
        if (hdr != nullptr) {
            int i = (hdr->getTlvOptions().findByType(IPv6TLVOPTION_TLV_GpsrBlackholeNormale));
            if (i >= 0)
                GpsrBlackholeNormaleOption = check_and_cast<GpsrBlackholeNormaleOption *>(hdr->getTlvOptionsForUpdate().getTlvOptionForUpdate(i));
        }
    }
    else
#endif
    ...
    if (auto nextHopHeader = dynamicPtrCast<NextHopForwardingHeader>(networkHeader)) {
        int i = (nextHopHeader->getTlvOptions().findByType(NEXTHOP_TLVOPTION_TLV_GpsrBlackholeNormale));
        if (i >= 0)
            GpsrBlackholeNormaleOption = check_and_cast<GpsrBlackholeNormaleOption *>(nextHopHeader->getTlvOptionsForUpdate().getTlvOptionForUpdate(i));
    }
    else
#endif
    {
...
const GpsrBlackholeNormaleOption *GpsrBlackholeNormale::getGpsrBlackholeNormaleOptionFromNetworkDatagram(const Ptr<const NetworkHeaderBase>& networkHeader) const
{
    const GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption = findGpsrBlackholeNormaleOptionInNetworkDatagram(networkHeader);
    if (GpsrBlackholeNormaleOption == nullptr)
        throw cRuntimeError("GpsrBlackholeNormale option not found in datagram!");
    return GpsrBlackholeNormaleOption;
}

GpsrBlackholeNormaleOption *GpsrBlackholeNormale::getGpsrBlackholeNormaleOptionFromNetworkDatagramForUpdate(const Ptr<NetworkHeaderBase>& networkHeader)
{
    GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption = findGpsrBlackholeNormaleOptionInNetworkDatagramForUpdate(networkHeader);
    if (GpsrBlackholeNormaleOption == nullptr)
        throw cRuntimeError("GpsrBlackholeNormale option not found in datagram!");
    return GpsrBlackholeNormaleOption;
}
...
INetfilter::IHook::Result GpsrBlackholeNormale::datagramPreRoutingHook(Packet *datagram)
{
    ...
    if (destination.isMulticast() || destination.isBroadcast() || routingTable->isLocalAddress(destination))
        ...
        // KLUDGE: this allows overwriting the GpsrBlackholeNormale option inside
        auto GpsrBlackholeNormaleOption = const_cast<GpsrBlackholeNormaleOption *>(getGpsrBlackholeNormaleOptionFromNetworkDatagram(networkHeader));
        return routeDatagram(datagram, GpsrBlackholeNormaleOption);
    }
}

INetfilter::IHook::Result GpsrBlackholeNormale::datagramLocalOutHook(Packet *packet)
{
    ...
    if (destination.isMulticast() || destination.isBroadcast() || routingTable->isLocalAddress(destination))
        ...
        GpsrBlackholeNormaleOption *GpsrBlackholeNormaleOption = createGpsrBlackholeNormaleOption(networkHeader->getDestinationAddress());
        setGpsrBlackholeNormaleOptionOnNetworkDatagram(packet, networkHeader, GpsrBlackholeNormaleOption);
        return routeDatagram(packet, GpsrBlackholeNormaleOption);
    }
}
...
void GpsrBlackholeNormale::handleStartOperation(LifecycleOperation *operation)
{
    configureInterfaces();
    storeSelfPositionInGlobalRegistry();
    scheduleBeaconTimer();
...
void GpsrBlackholeNormale::handleStopOperation(LifecycleOperation *operation)
{
...
void GpsrBlackholeNormale::handleCrashOperation(LifecycleOperation *operation)
{
    neighborPositionTable.clear();
    cancelEvent(beaconTimer);
    cancelEvent(purgeNeighborsTimer);
...
void GpsrBlackholeNormale::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
